...
int main(){
	...
	rep(i, 2187){
		...
		rep(gid, 3){
			ll exp = 0;
			ll lik = 0;
			for(ll hid = 0; hid<7; hid ++) if(a[hid]==gid){
				exp ++;
				rep(hid2, 7) if(a[hid2]==gid) lik += g[hid][hid2];
			}
			exp = s[gid] / exp;
			if(exp > expmax) expmax = exp;
			if(exp < expmin) expmin = exp;
			sumlik += lik;
		}
		ll expdiff = (expmax-expmin);
		if(expdiff < mnxp){
			mnxp = expdiff;
			mxlk = sumlik;
		}
		else if(expdiff == mnxp) if(sumlik > mxlk) mxlk = sumlik;
	}
	cout << mnxp << " " << mxlk << "\n";
    return 0;
}