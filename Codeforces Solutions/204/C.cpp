...
int main(){
    ...
    while (t--){
        ...
        vll v1[26], v2[26];
        rep(i, n){
            v1[a[i] - 'A'].emplace_back(i + 1);
		    v2[b[i] - 'A'].emplace_back(i + 1);
        }
        ld ans = 0;
        rep(i, 26){
            ll p1 = 0, p2 = 0, mul = 0;
            while (p1 < v1[i].size()){
                while (p2 < v2[i].size() and v2[i][p2] <= v1[i][p1]){
                    mul += v2[i][p2];
                    ++p2;
                }
                ans += 1.0l * mul * (n - v1[i][p1] + 1);
                ++p1;
            }
            p1 = v1[i].size() - 1;
            p2 = v2[i].size() - 1;
            mul = 0;
            while (p1 >= 0){
                while (p2 >= 0 and v2[i][p2] > v1[i][p1]){
                    mul += (n - v2[i][p2] + 1);
                    --p2;
                }
                ans += 1.0l * v1[i][p1] * mul;
                --p1;
            }
        }
        ans /= 1.0l * n * (n + 1) * (2*n + 1);
        ans *= 6.0;
