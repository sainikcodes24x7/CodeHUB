...
vpll v; sll mx, my;
bool fl(){
    if(mx.size() != 3 || my.size() != 3) return false;
    if(v[0].second != v[3].second || v[0].second != v[5].second) return false;
    if(v[1].second != v[6].second) return false;
    if(v[2].second != v[4].second || v[2].second != v[7].second) return false;
    return true;
}
...
int main(){
    ...
    while (t--){
        pll p[10];
        rep(i, 8) cin >> p[i].first >> p[i].second;
        sort(p + 0, p + 8);
        if (p[0].first == p[1].first && p[1].first == p[2].first 
            && p[0].second < p[1].second && p[1].second < p[2].second 
            && p[3].first == p[4].first && p[0].second == p[3].second && p[2].second == p[4].second 
            && p[5].first == p[6].first && p[6].first == p[7].first 
            && p[5].second == p[0].second && p[6].second == p[1].second && p[7].second == p[2].second) {
                cout << "respectable";
            } 
            else cout << "ugly";
    }    
