...
int main(){
    ...
    while (t--){
        ...
        ll pairwise_sum = 0;
        rep(i, k) {
            pairwise_sum += (1ll * i * a[i].first - running_sum);
            running_sum += a[i].first;
        }
        ll best = pairwise_sum;
	    ll ans = 0;
        rep_a(i, k, n){
            pairwise_sum -= (running_sum - 1ll * k * a[i - k].first);
			running_sum -= a[i-k].first;
			running_sum += a[i].first;
			pairwise_sum += (1ll * k * a[i].first - running_sum);
			if(pairwise_sum < best) {
				best = pairwise_sum;
				ans = i - k + 1;
			}
        }
        rep(i, k) cout << a[ans+i].second << " ";
    }    
    return 0;
}