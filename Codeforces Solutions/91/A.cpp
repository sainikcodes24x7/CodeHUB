...
int main(){
    ...
    while (t--){
        string s1, s2;
        cin >> s1 >> s2;
        vll d(26,-1); rep(i, 26) rep(j, 10001) place[i][j] = -1;
        rep(i, s1.size()){
            char c = s1[i];
            if (d[c - 'a'] == -1) rep(j, i + 1) place[c - 'a'][j] = i;
            else rep_a(j, d[c - 'a'] + 1, i + 1)  place[c - 'a'][j] = i;
            d[c - 'a'] = i;
        }
        ll res = 1, cur = 0;
        rep(i, s2.size()){
            if (cur==s1.size()) {cur = 0; res++;}
            char c  = s2[i] - 'a'; 
            if (place[c][cur] != -1) cur = place[c][cur] + 1;
            else{
                cur = 0; res++;
                if (place[c][cur] == -1) {res = -1; break;}
                else cur = place[c][cur] + 1;
            }
        }
        cout << res << "\n";
    }    
