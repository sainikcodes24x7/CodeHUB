...
void solve(ll &minv,ll &maxv,ll n,ll i,ll j,ll k){
    ll a = (i + 1) * (j + 2) * (k + 2);
    minv = min(minv, a-(n));
    maxv = max(maxv, a-(n));
}
    ...
    for(ll i = 1; i * i <= n; i++){
        if(n % i == 0){
            ...
            for(ll j = 1; j * j <= m; j++) {
                if(m % j == 0){
                    ll k = m / j;
                    solve(minv, maxv, n, i, j, k);
                    solve(minv, maxv, n, i, k, j);
                    solve(minv, maxv, n, j, i, k);
                    solve(minv, maxv, n, j, k, i);
                    solve(minv, maxv, n, k, i, j);
                    solve(minv, maxv, n, k, j, i);
                }
            }
        }
    }
