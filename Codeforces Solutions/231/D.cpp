...
ll inBetween(ll x, ll x1, ll a, ll b){
	if(x1 > 0){
		if(x >= 0 && x <= x1) return 0;
        else{
			if(x < 0) return a;
            else return b;
		}
...
int main(){
    ...
    while (t--){
        ...
        ll ans = inBetween(x, x1, ar[5], ar[6]);
        ans += inBetween(y, y1, ar[1], ar[2]);
        ans += inBetween(z, z1, ar[3], ar[4]);
        cout << ans << "\n";
    }    
    return 0;
}